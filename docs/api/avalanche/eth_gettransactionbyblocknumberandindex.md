...
meta:
  - name: description
    content: eth_getTransactionByBlockNumberAndIndex JSON-RPC method for the Avalanche API available with examples in web3.js, web3.py, eth.rb, and cURL.
  - name: keywords
...
web3.eth.getTransactionFromBlock(19642145, 2, (err, block) => {
  console.log(block)
})
```
...
print(web3.eth.get_transaction_by_block(19642145, 2)) # Hex encoded parameters starting with "0x" are accepted as well.
```
...
response = client.eth_get_transaction_by_block_number_and_index("0x12BB721", 2)
...
curl -X POST "CHAINSTACK_NODE_URL" \
  ...
  --data '{"method":"eth_getTransactionByBlockNumberAndIndex","params":["0x12BB721", "0x2"],"id":1,"jsonrpc":"2.0"}'
```
