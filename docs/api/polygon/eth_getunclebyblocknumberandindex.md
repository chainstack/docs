...
meta:
  - name: description
    content: eth_getUncleByBlockNumberAndIndex JSON-RPC method for the Polygon API available with examples in web3.js, web3.py, eth.rb, and cURL.
  - name: keywords
    content: json rpc methods curl api web3.py web3.js eth.rb javascript python ruby polygon 
---

# Polygon eth_getUncleByBlockNumberAndIndex RPC method

Polygon API method that returns information about an uncle of a block by number and uncle index position.   
...
web3.eth.getUncle("32739009", 0, (err, uncle) => {
    console.log(uncle)
})
```
...
print(web3.eth.get_uncle_by_block("0x1F38EC1", 0))    
```
...
response = client.eth_get_uncle_count_by_block_number("0x1F38EC1")
...
curl -X POST "CHAINSTACK_NODE_URL" \
  ...
  --data '{"method":"eth_getUncleByBlockNumberAndIndex","params":["0x1F38EC1", "0x0"],"id":1, "jsonrpc":"2.0"}'
```
