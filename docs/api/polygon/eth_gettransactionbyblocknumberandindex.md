...
meta:
  - name: description
    content: eth_getTransactionByBlockNumberAndIndex JSON-RPC method for the Polygon API available with examples in web3.js, web3.py, eth.rb, and cURL.
  - name: keywords
    content: json rpc methods curl api web3.py web3.js eth.rb javascript python ruby polygon 
---

# Polygon eth_getTransactionByBlockNumberAndIndex RPC method

Polygon API method that returns information about a transaction given a block number and a transaction's index position.  
...
web3.eth.getTransactionFromBlock(32738491, 22, (err, block) => {
  console.log(block)
})
```
...
response = client.eth_get_transaction_by_block_number_and_index("0x1F38CBB", 22)
...
curl -X POST "CHAINSTACK_NODE_URL" \
  ...
  --data '{"method":"eth_getTransactionByBlockNumberAndIndex","params":["0x1F38CBB", "0x16"],"id":1,"jsonrpc":"2.0"}'
```
